...
export function EngagementTrendCard({ scope, title, score, delta, trendLabel, participation, data }: Props) {
    ...
    <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#f3f4ff] via-white to-[#eef9ff] p-6 shadow-xl ring-1 ring-slate-200">
      <div className="flex items-center justify-between text-sm font-semibold text-slate-900">
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-white/70 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-primary ring-1 ring-primary/10">
            {scope === "team" ? "Survey report" : "Engagement trend"}
          </span>
          <span className="text-slate-800">{title}</span>
        </div>
        <button className="text-xs font-semibold text-primary underline-offset-4 hover:underline">Посмотреть детали</button>
      </div>
      ...
      <div className="relative mt-5 h-56 w-full">
        ...
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute bottom-3 left-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500">Месяцы / спринты</div>
        </div>
        ...
        <div className="absolute -top-6 left-4 w-[240px] rounded-2xl bg-white px-4 py-3 shadow-xl ring-1 ring-slate-200">
          <div className="flex items-center justify-between text-xs font-semibold text-slate-500">
            <span>{scope === "team" ? "Engagement score" : "Индекс вовлечённости"}</span>
            {participation !== undefined && <span className="text-slate-600">Participation: {participation}%</span>}
          </div>
