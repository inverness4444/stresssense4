...
export function AiEngagementReportPanel({ open, onClose, report, onChangePeriod, locale }: PanelProps) {
      ...
      >
        <div className="flex items-start justify-between border-b border-slate-100 px-5 py-4">
          <div>
            ...
            <p className="text-sm text-slate-600">{dateFormatter.format(new Date(report.period.from))} — {dateFormatter.format(new Date(report.period.to))}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          <section className="rounded-3xl border border-slate-200 bg-slate-50/60 p-4 shadow-sm">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                ...
                <span className="text-sm font-semibold text-emerald-700">+{report.deltaStress.toFixed(1)} {unitLabel}</span>
              </div>
              <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                <div>
