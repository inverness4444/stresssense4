...
export async function getAccessibleUserIdsForManager(managerId: string, organizationId: string): Promise<string[]> {
  ...
  const reportIds = directReports.map((d: any) => d.id);
  ...
  const usersInManagedTeams = await prisma.userTeam.findMany({
    where: { teamId: { in: teamsManaged.map((t: any) => t.teamId) } },
    select: { userId: true },
  });

  const ids = new Set<string>();
  ids.add(managerId);
  reportIds.forEach((id: any) => ids.add(id));
  usersInManagedTeams.forEach((u: any) => ids.add(u.userId));
