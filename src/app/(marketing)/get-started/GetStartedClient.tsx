...
export default function GetStartedClient({ locale }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/demo-org", {
      ...
      body: JSON.stringify({ orgName: orgName || (isRu ? "Новая компания" : "New company"), email, locale, plan }),
    });
    if (!res.ok) {
      alert(isRu ? "Не удалось создать демо-аккаунт. Попробуйте ещё раз." : "Could not start the demo. Please try again.");
      return;
    }
